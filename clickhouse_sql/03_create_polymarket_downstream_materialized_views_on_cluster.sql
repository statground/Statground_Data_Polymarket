...
SELECT
    ...
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'volume'), 'null')) AS volume,
...
SELECT
    ...
    parseDateTime64BestEffortOrNull(nullIf(JSONExtractString(payload, 'end_date_utc'), ''), 3, 'UTC') AS end_date_utc,
    parseDateTime64BestEffortOrNull(nullIf(JSONExtractString(payload, 'closed_time_utc'), ''), 3, 'UTC') AS closed_time_utc,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'best_ask'), 'null')) AS best_ask,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'best_bid'), 'null')) AS best_bid,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'last_trade_price'), 'null')) AS last_trade_price,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'spread'), 'null')) AS spread,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'volume'), 'null')) AS volume,
...
SELECT
    ...
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'liquidity'), 'null')) AS liquidity,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'volume'), 'null')) AS volume,
    toFloat64OrNull(nullIf(JSONExtractRaw(payload, 'volume_24h'), 'null')) AS volume_24h,
    JSONExtract(payload, 'event_ids', 'Array(UInt64)') AS event_ids,
    JSONExtractString(payload, 'raw_json') AS raw_json,
    ingested_at AS ingested_at
FROM `Data_Prediction_Raw`.polymarket_events_local
