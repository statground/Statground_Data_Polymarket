...
CREATE TABLE IF NOT EXISTS `Data_Prediction_Log`.polymarket_kafka_parse_error_local
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/Data_Prediction_Log/polymarket_kafka_parse_error_local', '{replica}')
...
CREATE TABLE IF NOT EXISTS `Data_Prediction_Log`.polymarket_kafka_parse_error
ON CLUSTER statground_cluster
AS `Data_Prediction_Log`.polymarket_kafka_parse_error_local
ENGINE = Distributed('statground_cluster', 'Data_Prediction_Log', 'polymarket_kafka_parse_error_local', cityHash64(kafka_topic, kafka_partition, kafka_offset))
COMMENT 'Polymarket Kafka parse error distributed log table; read interface across statground_cluster; OLAP only; SSOT 아님';

CREATE TABLE IF NOT EXISTS `Data_Prediction_Mart`.polymarket_collection_stats_hourly_local
ON CLUSTER statground_cluster
(
...
ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{shard}/Data_Prediction_Mart/polymarket_collection_stats_hourly_local', '{replica}')
...
CREATE TABLE IF NOT EXISTS `Data_Prediction_Mart`.polymarket_collection_stats_hourly
ON CLUSTER statground_cluster
AS `Data_Prediction_Mart`.polymarket_collection_stats_hourly_local
ENGINE = Distributed('statground_cluster', 'Data_Prediction_Mart', 'polymarket_collection_stats_hourly_local', cityHash64(domain, service, entity, toUInt64(toUnixTimestamp(bucket_at))))
...
CREATE MATERIALIZED VIEW IF NOT EXISTS `Data_Prediction_Mart`.mv_polymarket_event_snapshot_to_collection_stats_hourly_local
ON CLUSTER statground_cluster
TO `Data_Prediction_Mart`.polymarket_collection_stats_hourly_local
AS
SELECT
...
ALTER TABLE `Data_Prediction_Mart`.mv_polymarket_event_snapshot_to_collection_stats_hourly_local
ON CLUSTER statground_cluster
...
CREATE MATERIALIZED VIEW IF NOT EXISTS `Data_Prediction_Mart`.mv_polymarket_market_snapshot_to_collection_stats_hourly_local
ON CLUSTER statground_cluster
TO `Data_Prediction_Mart`.polymarket_collection_stats_hourly_local
AS
SELECT
...
ALTER TABLE `Data_Prediction_Mart`.mv_polymarket_market_snapshot_to_collection_stats_hourly_local
ON CLUSTER statground_cluster
...
CREATE MATERIALIZED VIEW IF NOT EXISTS `Data_Prediction_Mart`.mv_polymarket_series_snapshot_to_collection_stats_hourly_local
ON CLUSTER statground_cluster
TO `Data_Prediction_Mart`.polymarket_collection_stats_hourly_local
AS
SELECT
...
ALTER TABLE `Data_Prediction_Mart`.mv_polymarket_series_snapshot_to_collection_stats_hourly_local
ON CLUSTER statground_cluster
...
WHERE (database = 'Data_Prediction_Log' AND name LIKE 'polymarket%')
   OR (database = 'Data_Prediction_Mart' AND name LIKE 'polymarket%')
